...
if(app.ExcitationSignalDropDown.Value=="MIDI")
    [y]=output_midi(x,frame_length,lpcOrder,overlap,window,duration_win);
elseif(app.ExcitationSignalDropDown.Value=="Music File")
    [y]=output_music(x,frame_length,lpcOrder,overlap,window,duration_win);
else
    [y]=output_residuals(x,frame_length,lpcOrder,overlap,window);
end
